...
include("saltpass.php");
	...
	$teacher_table = mysql_query("SELECT * FROM Teachers WHERE teacher_name = '" . $_POST['teacher_login'] . "'") or die(mysql_error());
	
	//Gives error if user dosen't exist
	$check3 = mysql_num_rows($teacher_table);
	if ($check3 == 0)
	{
		$_SESSION['$wrong_user_teacher'] = TRUE;
	}
	while ($info = mysql_fetch_array($teacher_table))
	{
