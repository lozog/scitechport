...
<h2>Now editing: At least one unique item about your SciTech experiences that shows evidence of growth from the previous grade. Include a completed reflection(s).</h2>
...

    {
		...
		mysql_query("UPDATE Progress SET 11d1 = 0 WHERE student_number = '$student_number'");
...

    {
		...
		mysql_query("UPDATE Progress SET 11d1 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysql_query("UPDATE Progress SET 11d1 = 2 WHERE student_number = '$student_number'");
