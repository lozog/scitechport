...
<h2>Now editing: Passport to Safety certificate</h2>
...

    {
        ...
        $myFile = "users/" . $student_number . "/10/10b3.html";
...

    {
		...
		mysqli_query($connection, "UPDATE Progress SET 10b3 = 0 WHERE student_number = '$student_number'");
...

    {
		...
		//mysqli_query($connection, "UPDATE Progress SET 10b3 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysqli_query($connection, "UPDATE Progress SET 10b3 = 2 WHERE student_number = '$student_number'");
