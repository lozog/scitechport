...
<h2>Now editing: At least one unique item involving community work and/or extracurricular involvement (i.e. volunteering, school clubs, councils) with a completed reflection(s).</h2>
...

    {
        ...
        $myFile = "users/" . $student_number . "/10/10c1.html";
...

    {
		...
		mysqli_query($connection, "UPDATE Progress SET 10c1 = 0 WHERE student_number = '$student_number'");
...

    {
		...
		//mysqli_query($connection, "UPDATE Progress SET 10c1 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysqli_query($connection, "UPDATE Progress SET 10c1 = 2 WHERE student_number = '$student_number'");
