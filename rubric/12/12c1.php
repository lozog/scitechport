...
<h2>Now editing: At least one unique item involving community work and/or extracurricular involvement (i.e. volunteering, school clubs, councils) with a completed reflection(s).</h2>
...

    {
		...
		mysql_query("UPDATE Progress SET 12c1 = 0 WHERE student_number = '$student_number'");
...

    {
		...
		mysql_query("UPDATE Progress SET 12c1 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysql_query("UPDATE Progress SET 12c1 = 2 WHERE student_number = '$student_number'");
