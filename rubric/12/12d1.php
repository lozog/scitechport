...
<h2>Now editing: At least one unique item about your SciTech experiences that shows evidence of growth from the previous grade. Include a completed reflection(s).</h2>
...

    {
        ...
        $myFile = "users/" . $student_number . "/12/12d1.html";
...

    {
		...
		mysqli_query($connection, "UPDATE Progress SET 12d1 = 0 WHERE student_number = '$student_number'");
...

    {
		...
		//mysqli_query($connection, "UPDATE Progress SET 12d1 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysqli_query($connection, "UPDATE Progress SET 12d1 = 2 WHERE student_number = '$student_number'");
