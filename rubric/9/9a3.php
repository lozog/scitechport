...
<h2>Now editing: Semester 1 and 2 Report Cards</h2>
    ...
    if (isset($_POST['editor']))
    {
        ...
        $myFile = "users/" . $student_number . "/9/9a3.html";
        $fh     = fopen($myFile, 'w');
    ...
    if ($fs == 0)
    {
		...
		mysqli_query($connection, "UPDATE Progress SET 9a3 = 0 WHERE student_number = '$student_number'");
    }
    else
    {
		$data = fread($fh, filesize($myFile));
		//mysqli_query($connection, "UPDATE Progress SET 9a3 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysqli_query($connection, "UPDATE Progress SET 9a3 = 2 WHERE student_number = '$student_number'");
