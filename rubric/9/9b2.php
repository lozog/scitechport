...
<body>
<h2>Now editing: Evidence of Science Fair</h2>
	<form method="post">
			<?php
    ...
    if (isset($_POST['editor']))
    {
        ...
        $myFile = "users/" . $student_number . "/9/9b2.html";
        $fh     = fopen($myFile, 'w');
    ...
    if ($fs == 0)
    {
		...
		mysqli_query($connection, "UPDATE Progress SET 9b2 = 0 WHERE student_number = '$student_number'");
    }
    else
    {
		$data = fread($fh, filesize($myFile));
		//mysqli_query($connection, "UPDATE Progress SET 9b2 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysqli_query($connection, "UPDATE Progress SET 9b2 = 2 WHERE student_number = '$student_number'");
