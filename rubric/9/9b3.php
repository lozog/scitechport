...
<body>
<h2>Now editing: Evidence of at least two completed technology projects (Pop Bottle Rocket, Sumo Robot, Pinhole Camera, CO2 Car, Adobe Flash Animation, etc.) with a completed reflection for each</h2>
	<form method="post">
			<?php
    ...
    if (isset($_POST['editor']))
    {
        ...
        $myFile = "users/" . $student_number . "/9/9b3.html";
        $fh     = fopen($myFile, 'w');
    ...
    if ($fs == 0)
    {
		...
		mysqli_query($connection, "UPDATE Progress SET 9b3 = 0 WHERE student_number = '$student_number'");
    }
    else
    {
		$data = fread($fh, filesize($myFile));
		//mysqli_query($connection, "UPDATE Progress SET 9b3 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysqli_query($connection, "UPDATE Progress SET 9b3 = 2 WHERE student_number = '$student_number'");
