...
<h2>Now editing: Evidence of at least two completed technology projects (Pop Bottle Rocket, Sumo Robot, Pinhole Camera, CO2 Car, Adobe Flash Animation, etc.) with a completed reflection for each</h2>
    ...
    if ($fs == 0)
    {
		...
		mysql_query("UPDATE Progress SET 9b3 = 0 WHERE student_number = '$student_number'");
    }
    else
    {
		$data = fread($fh, filesize($myFile));
		mysql_query("UPDATE Progress SET 9b3 = 1 WHERE student_number = '$student_number'");
...
?></textarea>
		...
		if (isset($_POST['isDone'])){
			mysql_query("UPDATE Progress SET 9b3 = 2 WHERE student_number = '$student_number'");
